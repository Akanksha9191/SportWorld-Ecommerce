...
const GymAccessoriesPage = () => {
    const [gymAccessoriesdata, setGymAccessories] = useState([]);
    ...
    const FetchData = () => {
        Get(`http://localhost:8888/gymAccessoriesProduct`)
            .then((res) => {
                setGymAccessories(res.data);
            })
            .catch((error) => {
                console.log(error);
            });
    ...
    return (
        <>
            ...
            <div style={{ marginTop: '170px' }}>
                <h3 style={{ color: 'darkblue', margin: '20px' }}>Gym Accessories</h3>

                <div className="p-3">
                    <Row xs={1} sm={2} md={3} lg={4} className="g-4 mb-4">
                        {gymAccessoriesdata.map((item, idx) => {
...
export default GymAccessoriesPage;
